...
static struct option longopts[] = {{"host", required_argument, NULL, 'h'},
                                   ...
                                   {"timestamps", no_argument, NULL, 's'},
                                   {"type", no_argument, NULL, 't'},
                                   {"list-type", no_argument, NULL, 'e'},
...
static int parse_args(struct config* cfg, struct command* cmd, int argc, char** argv) {
  ...
  // parse option
  while ((c = getopt_long(argc, argv, "h:p:t:ltvse?", longopts, NULL)) != -1) {
    switch (c) {
      case 'h':
        cfg->host = optarg;
        break;
...
static void usage() {
  printf(
      "Usage: bpsp_c_client <options> command [arguments...]           \n"
      "  command:                                             \n"
      ...
      "    -e, --list-type           List type supported by client   \n"
      "    -h, --host             Broker host address         \n"
      "    -p, --port             Broker port address         \n"
      "    -l, --logs             Show logs                   \n"
      "    -s, --timestamps      Show logs timestamps       \n"
