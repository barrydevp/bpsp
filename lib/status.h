...
typedef enum {
    ...
    BPSP_MAX_VAR_HEADERS,  ///< Maximum variable heaaders
    ...
    BPSP_ILLEGAL_STATE,  ///< An unexpected state

    BPSP_SLOW_SUBSCRIBER,  ///< The maximum number of messages waiting to be
                           ///  delivered has been reached. Messages are dropped.

    BPSP_MAX_PAYLOAD,         ///< Attempt to send a payload larger than the maximum
                              ///  allowed by the BPSP Server.
    BPSP_MAX_DELIVERED_MSGS,  ///< Attempt to receive more messages than allowed

    BPSP_INSUFFICIENT_BUFFER,  ///< A buffer is not large enough to accommodate the data.

    BPSP_NO_MEMORY,  ///< An operation could not complete because of insufficient memory

    BPSP_SYS_ERROR,  ///< Some system function returned an error.

    BPSP_TIMEOUT,  ///< An operation timed-out.
    ...
    BPSP_NOT_YET_CONNECTED,  ///< A connection wasn't established
    BPSP_DRAINING,           ///< A connection and/or subscription entered the draining mode.
                             ///  Some operations will fail when in that mode.

    BPSP_INVALID_GROUP_NAME,  ///< An invalid group name was passed when creating a group subscription

    BPSP_NO_RESPONDERS,  ///< No responders were running when the server received the request.

    BPSP_MISMATCH,  ///< Request mismatch

    BPSP_MISSED_HEARTBEAT,  ///< For Heartbeat missed
